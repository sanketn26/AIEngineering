...
Before increasing a context limit, measure first-token latency and memory at expected concurrency. A request fitting the model's advertised window does not establish that twenty such requests fit the server. [Module 28](28-inference-serving.md#2-kv-cache-saved-computation-occupies-memory) works the memory arithmetic after Module 17. See the [runtime explanation](https://huggingface.co/docs/transformers/main/en/cache_explanation) for how cached state is reused.
